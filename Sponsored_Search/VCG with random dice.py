...
def calMarketWithMe(my_bid, my_value, other_bids, item_ctr):
    ...
    for i in range(0, len(bidList)):
     ...
     #get the random assignment with the random dice
    tie_breaking_index = np.random.randint(0, num_same)
    if tie_breaking_index == num_same:
        tie_breaking_index = tie_breaking_index - 1
    # implementation of the first half of the VCG function
    result = result - item_ctr[counter + tie_breaking_index] * my_bid + item_ctr[
        counter + tie_breaking_index] * my_value
...
'''
def vcg_random(end_time, n, bidders, item_ctr):
